...
const createProviderCreator = ( {
	useAuthHeaders, baseUrl,
} ) => ( recordType, extras={}, ) => {
	const Context = createContext();

	const Provider = ( { children, } ) => {
		const [ records, setRecords, ] = useState( null );
		const capitalizedRecordType = recordType[ 0 ].toUpperCase() + recordType.substring( 1 );
		...
		const value = {
			...
			[ `update${ capitalizedRecordType }` ]: function( id, record ) {
				return axios.put(
					value.getUrl( id ),
					record,
					{ headers: authHeaders, }
				...
				.then( () => {
					if ( records ) {
						const recordLocation = records.findIndex( record => id === record.id );
		...
		for ( const extraName in extras ) {
			value[ extraName ] = function() {
				return extras[ extraName ].apply( value, arguments );
			};
		}
